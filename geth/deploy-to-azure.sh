...
# Deploy Geth Monitoring to Azure Server with SSL Support
# This script will copy files and set up Geth on the remote server
...
print_header() {
    echo -e "${BLUE}╔════════════════════════════════════════╗${NC}"
    echo -e "${BLUE}║   Geth Azure Deployment Script        ║${NC}"
    echo -e "${BLUE}╚════════════════════════════════════════╝${NC}"
    echo ""
}
...
prompt_configuration() {
    ...
    read -p "Enter Azure Server IP [node-eth.nyyu.io]: " AZURE_HOST
    AZURE_HOST=${AZURE_HOST:-node-eth.nyyu.io}
    ...
    read -p "Enter remote directory [/home/azureuser/geth-monitoring]: " REMOTE_DIR
    REMOTE_DIR=${REMOTE_DIR:-/home/azureuser/geth-monitoring}
    ...
    if [[ "$USE_SSL" =~ ^[Yy]$ ]]; then
        read -p "Enter your domain name (e.g., geth.yourdomain.com): " DOMAIN
...
# Main server config
cat > nginx/conf.d/geth.conf << NGINX_GETH_EOF
...
server {
    ...
    location /mainnet/rpc {
        proxy_pass http://geth-mainnet:8545/;
    ...
    # Mainnet WebSocket
    location /mainnet/ws {
        proxy_pass http://geth-mainnet:8546/;
        proxy_http_version 1.1;
    ...
    location /sepolia/rpc {
        proxy_pass http://geth-sepolia:8545/;
        proxy_http_version 1.1;
        proxy_set_header Host \\\$host;
        proxy_set_header X-Real-IP \\\$remote_addr;
        proxy_set_header X-Forwarded-For \\\$proxy_add_x_forwarded_for;
    ...
    # Sepolia WebSocket
    location /sepolia/ws {
        proxy_pass http://geth-sepolia:8546/;
        proxy_http_version 1.1;
        proxy_set_header Upgrade \\\$http_upgrade;
        proxy_set_header Connection "upgrade";
        proxy_set_header Host \\\$host;
...
generate_jwt_secrets() {
    print_info "Generating JWT secrets for Geth-Lighthouse authentication..."

    # Create local jwt directories
    mkdir -p ./jwt/mainnet
    mkdir -p ./jwt/sepolia
    ...
    if [ ! -f ./jwt/mainnet/jwt.hex ]; then
        ...
        print_success "Generated JWT secret for mainnet"
    else
        print_success "JWT secret for mainnet already exists"
    fi

    # Generate JWT secret for sepolia (if it doesn't exist)
    if [ ! -f ./jwt/sepolia/jwt.hex ]; then
        openssl rand -hex 32 > ./jwt/sepolia/jwt.hex
        print_success "Generated JWT secret for sepolia"
    else
        print_success "JWT secret for sepolia already exists"
    fi
    ...
    # Upload JWT secrets to server
    print_info "Uploading JWT secrets to Azure server..."
    ssh -i "$SSH_KEY" "$AZURE_USER@$AZURE_HOST" "mkdir -p $REMOTE_DIR/jwt/mainnet $REMOTE_DIR/jwt/sepolia"
    scp -i "$SSH_KEY" ./jwt/mainnet/jwt.hex "$AZURE_USER@$AZURE_HOST:$REMOTE_DIR/jwt/mainnet/"
    scp -i "$SSH_KEY" ./jwt/sepolia/jwt.hex "$AZURE_USER@$AZURE_HOST:$REMOTE_DIR/jwt/sepolia/"

    print_success "JWT secrets ready for Geth-Lighthouse communication"
}
...
# Start services
start_services() {
    print_info "Starting Geth monitoring services..."
...
print_summary() {
    ...
    if [[ "$USE_SSL" =~ ^[Yy]$ ]]; then
        ...
        echo "  • Mainnet RPC:  http://$AZURE_HOST:8545"
        echo "  • Mainnet WS:   ws://$AZURE_HOST:8546"
        echo "  • Sepolia RPC:  http://$AZURE_HOST:8547"
        echo "  • Sepolia WS:   ws://$AZURE_HOST:8548"
        echo "  • Grafana:      http://$AZURE_HOST:3001 (admin/$GRAFANA_PASSWORD)"
        echo "  • Prometheus:   http://$AZURE_HOST:9091"
    fi

    echo ""
    echo -e "${YELLOW}Important Notes:${NC}"
    echo "  1. Initial sync will take 6-12 hours (Lighthouse checkpoint sync + Geth snap sync)"
    echo "  2. Both Geth (execution) and Lighthouse (consensus) clients must be running"
    echo "  3. Monitor sync: ssh -i $SSH_KEY $AZURE_USER@$AZURE_HOST 'cd $REMOTE_DIR && ./geth-manager.sh sync'"
    echo "  4. View logs: ssh -i $SSH_KEY $AZURE_USER@$AZURE_HOST 'cd $REMOTE_DIR && ./geth-manager.sh logs mainnet'"
    echo "  5. Check status: ssh -i $SSH_KEY $AZURE_USER@$AZURE_HOST 'cd $REMOTE_DIR && ./geth-manager.sh status'"
    ...
    echo "  1. Ensure Azure NSG allows ports 30303-30304 (P2P)"
    if [[ "$USE_SSL" =~ ^[Yy]$ ]]; then
