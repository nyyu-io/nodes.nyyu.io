...
# Deploy Polygon Monitoring to Azure Server with SSL Support
# This script will copy files and set up Polygon on the remote server
...
print_header() {
    echo -e "${BLUE}╔════════════════════════════════════════╗${NC}"
    echo -e "${BLUE}║   Polygon Azure Deployment Script     ║${NC}"
    echo -e "${BLUE}╚════════════════════════════════════════╝${NC}"
    echo ""
}
...
prompt_configuration() {
    ...
    read -p "Enter Azure Server IP [node-polygon.nyyu.io]: " AZURE_HOST
    AZURE_HOST=${AZURE_HOST:-node-polygon.nyyu.io}
    ...
    read -p "Enter remote directory [/home/azureuser/polygon-monitoring]: " REMOTE_DIR
    REMOTE_DIR=${REMOTE_DIR:-/home/azureuser/polygon-monitoring}
    ...
    if [[ "$USE_SSL" =~ ^[Yy]$ ]]; then
        read -p "Enter your domain name (e.g., polygon.yourdomain.com): " DOMAIN
...
create_env_file() {
    ...
    cat > .env.deploy << EOF
# Polygon Monitoring Configuration
...
ENDSSH
    ...
    rsync -avz --progress -e "ssh -i $SSH_KEY" \
        ...
        --exclude '.git' \
        --exclude 'deploy-to-azure-polygon.sh' \
        --exclude '.env.deploy' \
...
# Main server config
cat > nginx/conf.d/polygon.conf << NGINX_POLYGON_EOF
...
server {
    ...
    location /mainnet/rpc {
        proxy_pass http://polygon-mainnet:8545/;
    ...
    # Mainnet WebSocket
    location /mainnet/ws {
        proxy_pass http://polygon-mainnet:8546/;
        proxy_http_version 1.1;
    ...
    # Mumbai RPC (HTTP)
    location /mumbai/rpc {
        proxy_pass http://polygon-mumbai:8545/;
        proxy_http_version 1.1;
        proxy_set_header Host \\\$host;
        proxy_set_header X-Real-IP \\\$remote_addr;
        proxy_set_header X-Forwarded-For \\\$proxy_add_x_forwarded_for;
    ...
    # Mumbai WebSocket
    location /mumbai/ws {
        proxy_pass http://polygon-mumbai:8546/;
        proxy_http_version 1.1;
        proxy_set_header Upgrade \\\$http_upgrade;
        proxy_set_header Connection "upgrade";
        proxy_set_header Host \\\$host;
...
# Start services
start_services() {
    print_info "Starting Polygon monitoring services..."
...
print_summary() {
    ...
    if [[ "$USE_SSL" =~ ^[Yy]$ ]]; then
        ...
        echo "  • Polygon Mainnet RPC:  http://$AZURE_HOST:8545"
        echo "  • Polygon Mainnet WS:   ws://$AZURE_HOST:8546"
        echo "  • Polygon Mumbai RPC:   http://$AZURE_HOST:8547"
        echo "  • Polygon Mumbai WS:    ws://$AZURE_HOST:8548"
        echo "  • Grafana:              http://$AZURE_HOST:3006 (admin/$GRAFANA_PASSWORD)"
        echo "  • Prometheus:           http://$AZURE_HOST:9096"
    fi

    echo ""
    echo -e "${YELLOW}Important Notes:${NC}"
    echo "  1. Initial snap sync will take 4-8 hours for mainnet (~500GB)"
    echo "  2. Mumbai sync is faster (1-3 hours, ~200GB)"
    echo "  3. Polygon uses Bor client (fork of Geth)"
    echo "  4. Monitor sync: ssh -i $SSH_KEY $AZURE_USER@$AZURE_HOST 'cd $REMOTE_DIR && ./polygon-manager.sh sync'"
    echo "  5. View logs: ssh -i $SSH_KEY $AZURE_USER@$AZURE_HOST 'cd $REMOTE_DIR && ./polygon-manager.sh logs mainnet'"
    echo "  6. Check status: ssh -i $SSH_KEY $AZURE_USER@$AZURE_HOST 'cd $REMOTE_DIR && ./polygon-manager.sh status'"
    ...
    if [[ "$USE_SSL" =~ ^[Yy]$ ]]; then
        echo "  curl -X POST https://$DOMAIN/mainnet/rpc -H 'Content-Type: application/json' -d '{\"jsonrpc\":\"2.0\",\"method\":\"eth_blockNumber\",\"params\":[],\"id\":1}'"
        echo "  curl -X POST https://$DOMAIN/mumbai/rpc -H 'Content-Type: application/json' -d '{\"jsonrpc\":\"2.0\",\"method\":\"eth_blockNumber\",\"params\":[],\"id\":1}'"
    else
        echo "  curl -X POST http://$AZURE_HOST:8545 -H 'Content-Type: application/json' -d '{\"jsonrpc\":\"2.0\",\"method\":\"eth_blockNumber\",\"params\":[],\"id\":1}'"
        echo "  curl -X POST http://$AZURE_HOST:8547 -H 'Content-Type: application/json' -d '{\"jsonrpc\":\"2.0\",\"method\":\"eth_blockNumber\",\"params\":[],\"id\":1}'"
    fi
