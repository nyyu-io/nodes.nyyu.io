...
# Deploy Bitcoin Monitoring to Azure Server with SSL Support
# This script will copy files and set up Bitcoin Core on the remote server
...
print_header() {
    echo -e "${BLUE}╔════════════════════════════════════════╗${NC}"
    echo -e "${BLUE}║   Bitcoin Azure Deployment Script     ║${NC}"
    echo -e "${BLUE}╚════════════════════════════════════════╝${NC}"
    echo ""
}
...
prompt_configuration() {
    ...
    read -p "Enter Azure Server IP [node-btc.nyyu.io]: " AZURE_HOST
    AZURE_HOST=${AZURE_HOST:-node-btc.nyyu.io}

    read -p "Enter SSH username [azureuser]: " AZURE_USER
    AZURE_USER=${AZURE_USER:-azureuser}

    read -p "Enter SSH key path [./Azure-NYYU.pem]: " SSH_KEY
    SSH_KEY=${SSH_KEY:-./Azure-NYYU.pem}

    read -p "Enter remote directory [/home/azureuser/btc-monitoring]: " REMOTE_DIR
    REMOTE_DIR=${REMOTE_DIR:-/home/azureuser/btc-monitoring}

    echo ""

    # Bitcoin RPC Configuration
    read -p "Enter Bitcoin RPC username [rpcuser]: " BTC_RPC_USER
    BTC_RPC_USER=${BTC_RPC_USER:-rpcuser}

    read -p "Enter Bitcoin RPC password [rpcpassword]: " BTC_RPC_PASSWORD
    ...
    echo "  1) Pruned node (~100-150GB, recommended for most users)"
    echo "  2) Full archival node (~600GB+, for historical data)"
    read -p "Enter choice [1]: " NODE_TYPE
    NODE_TYPE=${NODE_TYPE:-1}

    if [ "$NODE_TYPE" == "2" ]; then
        PRUNE_MODE="0"
        PRUNE_DESC="Full archival node"
    else
        PRUNE_MODE="100000"
        PRUNE_DESC="Pruned node (~100GB)"
    fi
    ...
    if [[ "$USE_SSL" =~ ^[Yy]$ ]]; then
        read -p "Enter your domain name (e.g., btc.yourdomain.com): " DOMAIN
    ...
    echo "  Node Type: $PRUNE_DESC"
    echo "  RPC User: $BTC_RPC_USER"
    echo "  RPC Password: ${BTC_RPC_PASSWORD//?/*}"
...
# Update Bitcoin configuration with pruning mode
update_bitcoin_config() {
    print_info "Updating Bitcoin configuration..."

    # Update mainnet config
    if [ "$PRUNE_MODE" == "0" ]; then
        # Full node - comment out prune
        sed -i.bak 's/^prune=/#prune=/' config/mainnet/bitcoin.conf
    else
        # Pruned node - set prune value
        sed -i.bak "s/^prune=.*/prune=$PRUNE_MODE/" config/mainnet/bitcoin.conf
    fi

    # Update testnet config
    if [ "$PRUNE_MODE" == "0" ]; then
        sed -i.bak 's/^prune=/#prune=/' config/testnet/bitcoin.conf
    else
        sed -i.bak "s/^prune=.*/prune=$PRUNE_MODE/" config/testnet/bitcoin.conf
    fi

    # Update RPC credentials in both configs
    sed -i.bak "s/^rpcuser=.*/rpcuser=$BTC_RPC_USER/" config/mainnet/bitcoin.conf
    sed -i.bak "s/^rpcpassword=.*/rpcpassword=$BTC_RPC_PASSWORD/" config/mainnet/bitcoin.conf
    sed -i.bak "s/^rpcuser=.*/rpcuser=$BTC_RPC_USER/" config/testnet/bitcoin.conf
    sed -i.bak "s/^rpcpassword=.*/rpcpassword=$BTC_RPC_PASSWORD/" config/testnet/bitcoin.conf

    # Remove backup files
    rm -f config/mainnet/bitcoin.conf.bak config/testnet/bitcoin.conf.bak

    print_success "Bitcoin configuration updated"
}
...
create_env_file() {
    ...
    cat > .env.deploy << EOF
# Bitcoin Monitoring Configuration
...
ENDSSH
    ...
    rsync -avz --progress -e "ssh -i $SSH_KEY" \
        ...
        --exclude '.git' \
        --exclude 'deploy-to-azure-btc.sh' \
        --exclude '.env.deploy' \
...
# Setup SSL certificates (same as geth/bsc)
setup_ssl() {
    if [[ ! "$USE_SSL" =~ ^[Yy]$ ]]; then
        return
    fi
...
# Main server config for Bitcoin
cat > nginx/conf.d/btc.conf << NGINX_BTC_EOF
...
server {
    ...
    location /mainnet/rpc {
        proxy_pass http://btc-mainnet:8332/;
    ...
    location /testnet/rpc {
        proxy_pass http://btc-testnet:18332/;
        proxy_http_version 1.1;
        proxy_set_header Host \\\$host;
        proxy_set_header X-Real-IP \\\$remote_addr;
        proxy_set_header X-Forwarded-For \\\$proxy_add_x_forwarded_for;
...
# Start services
start_services() {
    print_info "Starting Bitcoin monitoring services..."
...
print_summary() {
    ...
    if [[ "$USE_SSL" =~ ^[Yy]$ ]]; then
        ...
        echo "  • Bitcoin Mainnet RPC:  http://$AZURE_HOST:8332"
        echo "  • Bitcoin Testnet RPC:  http://$AZURE_HOST:18332"
        echo "  • Grafana:              http://$AZURE_HOST:3003 (admin/$GRAFANA_PASSWORD)"
        echo "  • Prometheus:           http://$AZURE_HOST:9093"
    fi

    echo ""
    echo -e "${YELLOW}Important Notes:${NC}"
    if [ "$PRUNE_MODE" == "0" ]; then
        echo "  1. Full archival node selected (~600GB+ disk space required)"
        echo "  2. Initial blockchain download: 3-7 days"
    else
        echo "  1. Pruned node selected (~100GB disk space required)"
        echo "  2. Initial blockchain download: 12-24 hours"
    fi
    echo "  3. Monitor sync: ssh -i $SSH_KEY $AZURE_USER@$AZURE_HOST 'cd $REMOTE_DIR && ./btc-manager.sh sync'"
    echo "  4. View logs: ssh -i $SSH_KEY $AZURE_USER@$AZURE_HOST 'cd $REMOTE_DIR && ./btc-manager.sh logs mainnet'"
    echo "  5. Check status: ssh -i $SSH_KEY $AZURE_USER@$AZURE_HOST 'cd $REMOTE_DIR && ./btc-manager.sh status'"
    echo "  6. RPC credentials: $BTC_RPC_USER / ${BTC_RPC_PASSWORD//?/*}"
    ...
    echo "  1. Ensure Azure NSG allows ports 8333 and 18333 (P2P)"
    if [[ "$USE_SSL" =~ ^[Yy]$ ]]; then
