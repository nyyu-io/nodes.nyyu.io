...
# Deploy BSC Monitoring to Azure Server with SSL Support
# This script will copy files and set up BSC on the remote server
...
print_header() {
    echo -e "${BLUE}╔════════════════════════════════════════╗${NC}"
    echo -e "${BLUE}║   BSC Azure Deployment Script        ║${NC}"
    echo -e "${BLUE}╚════════════════════════════════════════╝${NC}"
    echo ""
}
...
prompt_configuration() {
    ...
    read -p "Enter Azure Server IP [node-bsc.nyyu.io]: " AZURE_HOST
    AZURE_HOST=${AZURE_HOST:-node-bsc.nyyu.io}
    ...
    read -p "Enter remote directory [/home/azureuser/bsc-monitoring]: " REMOTE_DIR
    REMOTE_DIR=${REMOTE_DIR:-/home/azureuser/bsc-monitoring}
    ...
    if [[ "$USE_SSL" =~ ^[Yy]$ ]]; then
        read -p "Enter your domain name (e.g., bsc.yourdomain.com): " DOMAIN
...
# Main server config
cat > nginx/conf.d/bsc.conf << NGINX_GETH_EOF
...
server {
    ...
    location /mainnet/rpc {
        proxy_pass http://bsc-mainnet:8545/;
    ...
    # Mainnet WebSocket
    location /mainnet/ws {
        proxy_pass http://bsc-mainnet:8546/;
        proxy_http_version 1.1;
    ...
    location /testnet/rpc {
        proxy_pass http://bsc-testnet:8545/;
        proxy_http_version 1.1;
        proxy_set_header Host \\\$host;
        proxy_set_header X-Real-IP \\\$remote_addr;
        proxy_set_header X-Forwarded-For \\\$proxy_add_x_forwarded_for;
    ...
    # Testnet WebSocket
    location /testnet/ws {
        proxy_pass http://bsc-testnet:8546/;
        proxy_http_version 1.1;
        proxy_set_header Upgrade \\\$http_upgrade;
        proxy_set_header Connection "upgrade";
        proxy_set_header Host \\\$host;
...
# Start services
start_services() {
    print_info "Starting BSC monitoring services..."
...
print_summary() {
    ...
    if [[ "$USE_SSL" =~ ^[Yy]$ ]]; then
        ...
        echo "  • BSC Mainnet RPC:  https://$DOMAIN/mainnet/rpc"
        echo "  • BSC Mainnet WS:   wss://$DOMAIN/mainnet/ws"
        echo "  • BSC Testnet RPC:  https://$DOMAIN/testnet/rpc"
        echo "  • BSC Testnet WS:   wss://$DOMAIN/testnet/ws"
        echo "  • Grafana:          https://$DOMAIN/grafana/"
        echo "  • Prometheus:       https://$DOMAIN/prometheus/"
    else
        echo -e "${BLUE}Access Points (HTTP):${NC}"
        echo "  • BSC Mainnet RPC:  http://$AZURE_HOST:8575"
        echo "  • BSC Mainnet WS:   ws://$AZURE_HOST:8576"
        echo "  • BSC Testnet RPC:  http://$AZURE_HOST:8577"
        echo "  • BSC Testnet WS:   ws://$AZURE_HOST:8578"
        echo "  • Grafana:          http://$AZURE_HOST:3002 (admin/$GRAFANA_PASSWORD)"
        echo "  • Prometheus:       http://$AZURE_HOST:9092"
    fi

    echo ""
    echo -e "${YELLOW}Important Notes:${NC}"
    echo "  1. Initial sync will take 4-8 hours for BSC snap sync"
    echo "  2. BSC uses Proof of Stake Authority (no consensus client needed)"
    echo "  3. Monitor sync: ssh -i $SSH_KEY $AZURE_USER@$AZURE_HOST 'cd $REMOTE_DIR && ./bsc-manager.sh sync'"
    echo "  4. View logs: ssh -i $SSH_KEY $AZURE_USER@$AZURE_HOST 'cd $REMOTE_DIR && ./bsc-manager.sh logs mainnet'"
    echo "  5. Check status: ssh -i $SSH_KEY $AZURE_USER@$AZURE_HOST 'cd $REMOTE_DIR && ./bsc-manager.sh status'"
