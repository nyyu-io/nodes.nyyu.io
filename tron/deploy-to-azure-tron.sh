...
# Deploy Tron Monitoring to Azure Server with SSL Support
# This script will copy files and set up Tron on the remote server
...
print_header() {
    echo -e "${BLUE}╔════════════════════════════════════════╗${NC}"
    echo -e "${BLUE}║   Tron Azure Deployment Script        ║${NC}"
    echo -e "${BLUE}╚════════════════════════════════════════╝${NC}"
    echo ""
}
...
prompt_configuration() {
    ...
    read -p "Enter Azure Server IP [node-tron.nyyu.io]: " AZURE_HOST
    AZURE_HOST=${AZURE_HOST:-node-tron.nyyu.io}
    ...
    read -p "Enter remote directory [/home/azureuser/tron-monitoring]: " REMOTE_DIR
    REMOTE_DIR=${REMOTE_DIR:-/home/azureuser/tron-monitoring}
    ...
    if [[ "$USE_SSL" =~ ^[Yy]$ ]]; then
        read -p "Enter your domain name (e.g., tron.yourdomain.com): " DOMAIN
...
ENDSSH
    ...
    rsync -avz --progress -e "ssh -i $SSH_KEY" \
        ...
        --exclude '.git' \
        --exclude 'deploy-to-azure-tron.sh' \
        --exclude '.env.deploy' \
...
# Allow Tron P2P ports
sudo ufw allow 18888/tcp
sudo ufw allow 18888/udp
sudo ufw allow 18889/tcp
sudo ufw allow 18889/udp
...
if [ "$USE_SSL" == "y" ] || [ "$USE_SSL" == "Y" ]; then
    ...
    # Allow direct API access if no SSL
...
http {
    ...
    # Increase buffer sizes for Tron API responses
...
# Main server config
cat > nginx/conf.d/tron.conf << NGINX_TRON_EOF
...
server {
    ...
    location /mainnet/api/ {
        proxy_pass http://tron-mainnet:8090/;
    ...
    # Nile Testnet API
    location /nile/api/ {
        proxy_pass http://tron-nile:8091/;
        proxy_http_version 1.1;
        proxy_set_header Host \\\$host;
        proxy_set_header X-Real-IP \\\$remote_addr;
        proxy_set_header X-Forwarded-For \\\$proxy_add_x_forwarded_for;
...
# Start services
start_services() {
    print_info "Starting Tron monitoring services..."
...
print_summary() {
    ...
    if [[ "$USE_SSL" =~ ^[Yy]$ ]]; then
        ...
        echo "  • Tron Mainnet API:  http://$AZURE_HOST:8090"
        echo "  • Tron Nile API:     http://$AZURE_HOST:8091"
        echo "  • Grafana:           http://$AZURE_HOST:3004 (admin/$GRAFANA_PASSWORD)"
        echo "  • Prometheus:        http://$AZURE_HOST:9094"
    fi

    echo ""
    echo -e "${YELLOW}Important Notes:${NC}"
    echo "  1. Initial sync will take 2-4 days for Tron mainnet full node (~1.5TB)"
    echo "  2. Nile testnet sync is faster (~300GB full, ~100GB lite)"
    echo "  3. Tron nodes use HTTP API on ports 8090 (mainnet) and 8091 (nile)"
    echo "  4. Monitor sync: ssh -i $SSH_KEY $AZURE_USER@$AZURE_HOST 'cd $REMOTE_DIR && ./tron-manager.sh sync'"
    echo "  5. View logs: ssh -i $SSH_KEY $AZURE_USER@$AZURE_HOST 'cd $REMOTE_DIR && ./tron-manager.sh logs mainnet'"
    echo "  6. Check status: ssh -i $SSH_KEY $AZURE_USER@$AZURE_HOST 'cd $REMOTE_DIR && ./tron-manager.sh status'"
    ...
    echo "  1. Ensure Azure NSG allows ports 18888-18889 (P2P)"
    if [[ "$USE_SSL" =~ ^[Yy]$ ]]; then
        echo "  2. Configure DNS to point $DOMAIN to $AZURE_HOST"
        echo "  3. Wait for SSL certificate to be issued"
        echo "  4. Update your application's API URLs to use https://$DOMAIN"
    else
        echo "  2. Optionally configure SSL later by re-running with SSL options"
        echo "  3. Update your application's API URLs to use $AZURE_HOST"
    fi

    echo ""
    echo -e "${BLUE}Test API Endpoints:${NC}"
    if [[ "$USE_SSL" =~ ^[Yy]$ ]]; then
        echo "  curl https://$DOMAIN/mainnet/api/wallet/getnowblock"
        echo "  curl https://$DOMAIN/nile/api/wallet/getnowblock"
    else
        echo "  curl http://$AZURE_HOST:8090/wallet/getnowblock"
        echo "  curl http://$AZURE_HOST:8091/wallet/getnowblock"
    fi
