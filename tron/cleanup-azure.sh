...
AZURE_HOST="node-tron.nyyu.io"
AZURE_USER="azureuser"
SSH_KEY="./Azure-NYYU.pem"
REMOTE_DIR="/home/azureuser/tron-monitoring"

echo -e "${BLUE}╔════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║   Tron Azure Cleanup Script            ║${NC}"
echo -e "${BLUE}╚════════════════════════════════════════╝${NC}"
echo ""

echo -e "${YELLOW}WARNING: This will stop all Tron monitoring services!${NC}"
echo ""
read -p "Do you want to also remove all blockchain data? (y/N) [N]: " REMOVE_DATA
REMOVE_DATA=${REMOVE_DATA:-n}

if [[ "$REMOVE_DATA" =~ ^[Yy]$ ]]; then
    echo -e "${YELLOW}⚠ This will DELETE all blockchain data (~1.5TB). This cannot be undone!${NC}"
    echo "Re-syncing will take 2-4 days."
    echo ""
